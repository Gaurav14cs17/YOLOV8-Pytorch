...
class DFL(nn.Module):
    """Distribution Focal Loss module for box regression."""
...
class DetectionHead(nn.Module):
    ...
    def __init__(self, num_classes=80, in_channels=()):
        ...
        self.nc = num_classes  # Alias for ComputeLoss compatibility
        self.dfl_ch = 16
        self.output_size = num_classes + 4 * self.dfl_ch
        self.no = self.output_size  # outputs per anchor for ComputeLoss
        ...
        for ch in in_channels:
            # Box prediction branch (outputs 4 * dfl_ch for DFL)
            box_channels = max(64, ch // 4, self.dfl_ch * 4)
            box_branch = nn.Sequential(
                BlockBuilder.conv_bn_act(ch, box_channels, 3),
                BlockBuilder.conv_bn_act(box_channels, box_channels, 3),
                nn.Conv2d(box_channels, 4 * self.dfl_ch, 1)  # 64 outputs for DFL
            )
